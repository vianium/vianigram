...
{
    /// <summary>
    /// Cancel a transfer. Cancellation is cooperative — the chunk fan-out
    /// tasks observe <c>MediaTransferState.Cancelled</c> at the top of each
    /// retry iteration and short-circuit. Already-completed chunks remain
    /// in the aggregate (useful for a future resume-from-cancel feature).
    /// </summary>
    public sealed class CancelTransferHandler
    {
        ...
        public CancelTransferHandler(TransferRegistry registry, IEventBus bus, IClock clock, ILogger log, ITelemetry telemetry)
        {
            ...
            _bus = bus;
            _clock = clock;
            _log = new TimestampedLogger(log, "Media.CancelTransfer");
            _telemetry = telemetry;
        }

        public Task<Result<Domain.ValueObjects.Unit, MediaError>> HandleAsync(CancelTransferCommand cmd, CancellationToken ct)
        {
            if (cmd == null)
            ...
            transfer.Cancel();
            _bus.Publish(new TransferFailed(cmd.Id, "cancelled", _clock.UtcNow));
            _telemetry.Track("media.transfer.cancelled", 1);
