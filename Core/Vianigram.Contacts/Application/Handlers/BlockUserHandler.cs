...
{
    /// <summary>
    /// Issues <c>contacts.block#2e2e8734</c> against the configured RPC port,
    /// then mirrors the block in the local <see cref="ContactBook"/> aggregate.
    /// </summary>
    internal sealed class BlockUserHandler
    {
        ...
        public BlockUserHandler(IContactRepository repo, IMtProtoRpcPort rpc, IEventBus bus, ILogger log, IClock clock)
        {
            ...
            _rpc = rpc;
            _bus = bus;
            _log = new TimestampedLogger(log, "Contacts.BlockUser");
            _clock = clock;
        }

        public async Task<Result<Unit, ContactsError>> HandleAsync(BlockUserCommand cmd, CancellationToken ct)
        {
            ...
            byte[] request = TlEncoder.EncodeBlock(cmd.Target.Value, cmd.AccessHash, /*myStoriesFrom*/ false);
            var rpcResult = await _rpc.CallAsync(request, ct).ConfigureAwait(false);
            if (rpcResult.IsFail)
            {
                var mapped = RpcErrorMapper.Map(rpcResult.Error);
                _log.Warn("contacts.block failed: " + mapped);
                return Result<Unit, ContactsError>.Fail(mapped);
            }

            book.Block(cmd.Target, _clock.UtcNow);
