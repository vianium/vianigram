...
namespace Vianigram.Contacts.Ports.Outbound
{
    ...
    /// bounded contexts decoupled (Contacts does not reference Chats.Ports nor
    /// Account.Ports). The same concrete adapter (<c>MtProtoRpcAdapter</c>) in
    /// the host composition layer implements every per-context interface — the
    /// ACL pattern documented in <c>docs/managed-architecture/principles.md</c>.
    ///
    /// Contract:
