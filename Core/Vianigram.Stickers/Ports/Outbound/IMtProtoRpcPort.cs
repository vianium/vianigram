...
namespace Vianigram.Stickers.Ports.Outbound
{
    ...
    /// bounded contexts decoupled (Stickers does not reference Contacts.Ports
    /// nor Account.Ports). The same concrete adapter (<c>MtProtoRpcAdapter</c>)
    /// in the host composition layer implements every per-context interface —
    /// the ACL pattern documented in <c>docs/managed-architecture/principles.md</c>.
    ///
    /// Contract:
    ...
    ///     Network / cancellation errors must NOT throw across the port — they
    ///     are surfaced as <see cref="MtProtoRpcError"/> with <c>Kind="Network"</c>.
    /// </summary>
